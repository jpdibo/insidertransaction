...
def create_audit_sample(connection, source_id: str, size: int, output: Path) -> int:
    ...
    rows = connection.execute(
        ...
        "JOIN filing_publications fp ON fp.filing_version_id=fv.id JOIN source_records sr ON sr.id=fp.source_record_id "
    ...
    for row in rows:
        ...
        event = events.setdefault(item["event_id"], {key: item[key] for key in (
            ...
            "action", "nature_raw", "trade_date", "venue_raw", "instrument_name", "isin",
        )})
    ...
    fieldnames = [
        ...
        "party_name", "action", "nature_raw", "trade_date", "venue_raw", "instrument_name", "isin",
