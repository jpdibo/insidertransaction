...
Validation updated: 08 September 2026. Host: Windows. Runtime: Conda `short_selling`, CPython 3.10.18, SQLite 3.50.3, pypdf 3.17.4. Result: **44 passed, 0 failed, 0 skipped**.
...
Final Conda test output on 09 September 2026 ended with `Ran 46 tests` and `OK`. Compilation emitted no errors. Regression coverage includes correction-before-original ordering, removal-aware reparse retractions, abandoned-run finalization, lock replacement ownership, global nullable-identifier uniqueness, explicit cross-source duplicate links, UTC scheduler configuration, legacy German and English Unternehmensregister templates, Netherlands AFM XML/PCA/detail reconciliation, English/Danish OAM form fallbacks, deterministic audit samples, and the NewsWeb `681853` no-space PDF-label layout.
...
Canonical database checks after the 09 September scheduled run and Norway saved-evidence retry: integrity `ok`; 0 foreign-key violations; 322 current events; 536 event versions; 349 source records and 709 immutable raw objects. Current-event sources: Switzerland 36, Germany BaFin 20, Germany archive 27, Denmark 6, France 61, Netherlands 41, Norway 52, Sweden 73 and synthetic controls 6.
...
- Live discovery and stable repeat ingestion are demonstrated for Norway, Sweden, Germany BaFin and Unternehmensregister, Switzerland, France and Netherlands. Real correction-before-original, process termination injection, observed scheduler firing, and OCR extraction remain untested.
