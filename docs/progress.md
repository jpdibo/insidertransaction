...
Migration ledger/schema through 012, source registry, immutable raw storage, official adapters for seven jurisdictions, deterministic HTML/JSON/PDF/XML parsers, persisted discovery metadata and filing/group raw names, exact decimals, PCA relationships, anonymous roles, semantic document hashes, filing/event revisions and retractions, cross-source duplicate links, row reconciliation, dry-run outbox, ownership-safe writer lock, abandoned-run recovery, checkpoints, reports, online backup/restore, web/API/CSV, installed Conda scheduler and 50 passing tests. The canonical database has 362 current events. Netherlands has 41 current events. Denmark contributed six events from seven announcements; `300014006` is evidence-incomplete and remains quarantined. Norway has 52 current events after recovering `681853`; only `681380` and `681718` remain quarantined for missing authoritative evidence. Sweden has 113 current events after count-checked, daily-partitioned retrieval completed under measured FI pacing. Independent evidence audits now pass 50/50 for both France and Sweden.
