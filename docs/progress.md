...
Migration ledger/schema through 013, source registry, immutable raw storage, official adapters for seven jurisdictions, deterministic HTML/JSON/PDF/XML parsers, persisted discovery metadata, filing/group raw names and option-underlying identifiers, exact decimals, PCA relationships, anonymous roles, semantic document hashes, filing/event revisions and retractions, cross-source duplicate links, row reconciliation, dry-run outbox, ownership-safe writer lock, abandoned-run recovery, checkpoints, reports, online backup/restore, web/API/CSV, installed Conda scheduler and 51 passing tests. The canonical database has 396 current events. Netherlands has 62 current events from 53 notifications. Denmark contributed six events from seven announcements; `300014006` is evidence-incomplete and remains quarantined. Norway has 52 current events; only `681380` and `681718` remain quarantined for missing authoritative evidence. Sweden has 113 current events. BaFin has 33 events after bounded backfill through 27 August. Independent evidence audits pass 50/50 for France, Sweden, Netherlands and Norway.
